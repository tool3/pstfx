...
test('rejects a non-string source', () => {
  assert.throws(() => parse(42 as unknown as string), (error: VctrfxError) => error.code === 'INVALID_SOURCE')
})

test('rejects an empty source', () => {
  assert.throws(() => parse('   '), (error: VctrfxError) => error.code === 'INVALID_SOURCE')
})

test('rejects markup whose root is not an svg', () => {
  assert.throws(() => parse('<div><span/></div>'), (error: VctrfxError) => error.code === 'NOT_AN_SVG')
})
